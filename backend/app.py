...
class MineRiskDetector:
    ...
    def detect_risks(self, image):
        ...
        Args:
            image (np.ndarray): The OpenCV image (BGR format).
            
        Returns:
            list: List of detected risk objects.
        """
        detections = []
        
        # Check if image is valid (it must be a NumPy array here)
        if image is None or image.size == 0:
            logger.error("Received empty or invalid image (NumPy array) for detection.")
            return detections
        ...
        # Detect loose rocks using contour analysis
        blurred = cv2.GaussianBlur(gray, (5, 5), 0)
        # Use simple thresholding to segment bright/dark areas (potential rocks)
        thresh = cv2.threshold(blurred, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
        rock_contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        # Detect structural damage using edge detection (Simulated)
        structural_damage = self.detect_structural_damage(image)
        if structural_damage:
            detections.append(structural_damage)
        
        # Simulate gas leak detection
        if np.random.random() < 0.05:  # 5% chance of gas leak simulation
            # Choose a random bounding box area
            h, w, _ = image.shape
    ...
    def annotate_image(self, image, detections):
        """Annotate image with detection results"""
        # Ensure the image is in BGR format for OpenCV drawing functions
        annotated = image.copy()
        ...
        for detection in detections:
            ...
            # Get color and severity info (OpenCV uses BGR format)
            risk_info = self.risk_types.get(detection['type'], {'color': (128, 128, 128), 'severity': 'low'})
            color = risk_info['color']
...
def index():
    """Simple root endpoint for health/info."""
    return jsonify({
        'app_name': 'RockGuard AI Backend',
        'status': 'Running',
        'message': 'API endpoints start with /api/',
        'endpoints': ['/api/health', '/api/process_frame', '/api/detection_status', '/api/start_detection', '/api/stop_detection']
    })
...
def process_frame():
    ...
    try:
        ...
        # 1. Decode image data once (assuming it's a data URL: data:image/type;base64,...)
        base64_image_data = data['image']
        
        if 'base64,' not in base64_image_data:
            return jsonify({'error': "Image data is not in expected 'data:image/...;base64,...' format. Please provide a data URL."}), 400
            
        image_data_base64_only = base64_image_data.split(',')[1]
        image_data = base64.b64decode(image_data_base64_only)
        
        # 2. Convert to OpenCV format (NumPy array, BGR color channels)
        image_pil = Image.open(io.BytesIO(image_data))
        image_cv = cv2.cvtColor(np.array(image_pil), cv2.COLOR_RGB2BGR) 
        
        # 3. Perform detection and calculate risk
        detections = detector.detect_risks(image_cv) 
        risk_level = detector.calculate_risk_level(detections)
        ...
        # 5. Convert annotated image back to base64 for API response
        # Encode as JPEG for efficiency (Quality 85)
        _, buffer = cv2.imencode('.jpeg', annotated_image, [int(cv2.IMWRITE_JPEG_QUALITY), 85])
        annotated_base64 = base64.b64encode(buffer).decode('utf-8')
        
        # 6. Update global results and return response
        detection_results = detections
...
# If the server is run directly, start the Flask app
if __name__ == '__main__':
    # Create backend directory if it doesn't exist
    os.makedirs('backend', exist_ok=True)
    ...
    print("📡 Server will be available at: http://localhost:5000")
    print("🔍 AI Detection endpoints:")
    print("   - GET  / - Server status") # New endpoint added
    print("   - POST /api/process_frame - Process camera frame")
    print("   - GET  /api/health - Health check")
    print("   - POST /api/start_detection - Start detection")
    print("   - POST /api/stop_detection - Stop detection")
